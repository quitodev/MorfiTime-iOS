...
struct BookingView: View {
    ...
    init(viewModel: BookingViewModelImpl, userData: User) {
        ...
        bookingViewModel.getBookings(place: userData.place, userID: userData.id, userName: userData.name)
    }
                ...
                ScrollView {
                    ScrollViewReader { scrollViewReader in
                                        ...
                                        VStack {
                                            ScrollView {
                                                ForEach(bookingViewModel.successGetBookings.schedule, id: \.self) { schedule in
                                                    // AGREGAR QUE SI EL USER.BOOKINGS ES IGUAL O MAYOR A 30, MUESTRE UNA ALERTA DE LIMITE ALCANZADO
                        ...
                        .onAppear {
                            ...
                            isShowingDays = false
                            isShowingSchedule = false
                            bookingViewModel.getBookings(place: user.place, userID: user.id, userName: user.name)
